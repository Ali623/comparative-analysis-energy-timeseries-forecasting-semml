...
class ModelConfig(BaseModel):
    ...
    batch_size : int = Field(..., description="To write the batch size")
...
def create_batches(df: pd.DataFrame, batch_size: int):
    """
    Split a DataFrame with a single column into batches of a specified size.

    Args:
        df (pd.DataFrame): Input DataFrame with a single column.
        batch_size (int): Size of each batch.

    Returns:
        List[pd.DataFrame]: List of DataFrames, each of batch_size rows.
    """
    if df.shape[1] != 1:
...
def train_and_forecast(dataset: pd.DataFrame, config: ModelConfig):
    ...
    for i in reversed(range(config.n_splits)):
        ...
        data_train_batches = create_batches(data_train, config.batch_size)

    
        for j in range(int(config.training_horizon / config.batch_size)):

            hcnn = HCNN(data_dim=data_train_batches[j].shape[1], hidden_dim=100, sparsity=0, init_state_trainable=False)
            init_state = hcnn.init_state()

            train_loss = hcnn.train(
                data_train_batches[j].values, 
                init_state, 
                lr=0.0005, epochs=500, 
                criterion=LogCosh.apply, #LogCosh.apply, torch.nn.MSELoss()
                reduce_lr_epochs = 400, 
